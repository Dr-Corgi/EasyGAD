...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, multi_turn=False, norm_adv_by_std_in_grpo=True, config=None, compute_teacher=False):
    ...
    Args:
        ...
        compute_teacher (bool, optional): Whether to compute teacher advantages (for seqkd stage). Defaults to False.
    ...
    if compute_teacher:
        # For seqkd stage: compute teacher_response_mask
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        if compute_teacher:
            # For seqkd stage: use teacher data
            ...
            # For warmup/gad stage: use student data
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        # Setup training stage configuration
        self.training_stage = config.trainer.get("training_stage", "warmup")
        self._setup_stage_config(role_worker_mapping)
    ...
    def _setup_stage_config(self, role_worker_mapping):
        """Setup configuration based on training stage.
        ...
        if stage == "seqkd":
            # SeqKD: no reference policy, no critic (teacher forcing GRPO)
            self.use_reference_policy = False
            self.use_critic = False
        elif stage == "warmup":
            # Warmup: use critic as discriminator, SFT update
            self.use_reference_policy = False
        ...
        print(f"[Training Stage: {stage}] use_reference_policy={self.use_reference_policy}, use_critic={self.use_critic}")
    ...
    def _forward_batch_teacher_forcing_grpo(self, batch, teacher_repeat):
        """Compute teacher values for GRPO using teacher forcing (seqkd stage).

        This creates values for teacher responses where each response in a group
        gets a value based on its position (0, 1, 2, ...) for GRPO advantage computation.
        """
    ...
    def compute_teacher_values(self, data: DataProto):
        """Compute teacher values for seqkd stage using teacher forcing GRPO."""
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # Compute response mask based on training stage
                    ...
                    # recompute old_log_probs (not needed for seqkd stage)
                    if self.training_stage != "seqkd":
                    ...
                    # For seqkd stage, compute teacher values using teacher forcing GRPO
                    with marked_timer("reward", timing_raw, color="yellow"):
                        future_reward = None
                        reward_extra_infos_dict = {}
                        if self.training_stage == "seqkd":
                            # Seqkd: compute teacher values via teacher forcing GRPO
                            batch.meta_info["compute_teacher"] = True
                            batch.meta_info["teacher_repeat"] = self.config.actor_rollout_ref.rollout.n
                            teacher_values = self.compute_teacher_values(batch)
                            batch = batch.union(teacher_values)
                            reward_tensor = batch.batch["teacher_values"]
                        else:
                            # Warmup/GAD: use critic as discriminator
                            batch.meta_info["compute_teacher"] = False
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        if self.training_stage == "seqkd":
                            # Seqkd: use teacher values as token level rewards
                            batch.batch["teacher_token_level_rewards"] = reward_tensor
                        else:
                            # Warmup/GAD: use discriminator values as token level scores
                        ...
                        if self.config.algorithm.use_kl_in_reward and self.training_stage != "seqkd":
                            batch, kl_metrics = apply_kl_penalty(batch, kl_ctrl=self.kl_ctrl_in_reward, kl_penalty=self.config.algorithm.kl_penalty)
                            metrics.update(kl_metrics)
                        elif self.training_stage != "seqkd":
                        ...
                        batch = compute_advantage(
                            ...
                            compute_teacher=(self.training_stage == "seqkd"),
                        )

                    # update critic (discriminator) - only for warmup/gad stages
                    if self.use_critic and self.training_stage in ["warmup", "gad"]:
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        ...
                        with marked_timer("update_actor", timing_raw, color="red"):
                            ...
                            batch.meta_info["training_stage"] = self.training_stage
