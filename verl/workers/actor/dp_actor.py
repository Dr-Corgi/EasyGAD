...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto):
        ...
        if training_stage in ["seqkd", "warmup"]:
            # SFT mode: use teacher response
            select_keys = [
        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                if has_multi_modal_inputs:
                    ...
                    max_token_len = self.config.ppo_max_token_len_per_gpu * self.ulysses_sequence_parallel_size
                    micro_batches, _ = rearrange_micro_batches(batch=mini_batch, max_token_len=max_token_len, compute_teacher=compute_teacher)
                else:
                    self.gradient_accumulation = self.config.ppo_mini_batch_size // self.config.ppo_micro_batch_size_per_gpu
                    # split batch into micro_batches
                ...
                for data in micro_batches:
                    ...
                    if training_stage in ["seqkd", "warmup"]:
                        ...
                        if multi_turn:
                            teacher_response_mask = data["loss_mask"][:, -teacher_response_length:]
                        else:
                        ...
                        if self.config.use_dynamic_bsz:
                            loss = pg_loss * (len(data["teacher_response"]) / self.config.ppo_mini_batch_size)
                        else:
                        ...
                        if self.config.use_dynamic_bsz:
                            loss = policy_loss * (len(data["responses"]) / self.config.ppo_mini_batch_size)
                        else:
